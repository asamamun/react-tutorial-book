...
const API = `http://192.168.54.78/r49/laravel/projects/TigerQuiz/public/api/questions`;
const questions=[];
        fetch(API)
        .then(data=>data.json())
        .then(data=>{
        ...
        function showdata(qs){
          // console.log(qs);
          for (const key in qs['q']) {
            if (qs['q'].hasOwnProperty.call(qs['q'], key)) {
              const element = qs['q'][key];
              console.log(element);              
            }
          }
        }
...
/*         fetch('http://192.168.54.78/r49/laravel/projects/TigerQuiz/public/api/questions')
        .then(data=>data.json())
